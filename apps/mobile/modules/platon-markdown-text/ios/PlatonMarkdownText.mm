...
#import "PlatonMarkdownTextShadowNode.h"
#import "PlatonMarkdownTextComponentDescriptor.h"
...
static void PlatonMarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<PlatonMarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void PlatonMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<PlatonMarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = PlatonMarkdownTextAttachmentSize(attachmentRange);
    attachment.bounds = CGRectMake(
        0,
        PlatonMarkdownTextAttachmentBaselineOffset(attachmentRange),
        attachmentSize,
        attachmentSize);
...
@protocol PlatonMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface PlatonMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<PlatonMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<PlatonMarkdownOutsideTapTarget>)target;

@end

static const void *PlatonMarkdownOutsideTapCoordinatorKey =
    &PlatonMarkdownOutsideTapCoordinatorKey;

@implementation PlatonMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<PlatonMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<PlatonMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, PlatonMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<PlatonMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static PlatonMarkdownOutsideTapCoordinator *
PlatonMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  PlatonMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, PlatonMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[PlatonMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        PlatonMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface PlatonMarkdownText () <RCTPlatonMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface PlatonMarkdownText () <PlatonMarkdownOutsideTapTarget>
@end

@implementation PlatonMarkdownText {
  UIView * _view;
  UITextView * _textView;
  PlatonMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<PlatonMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const PlatonMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    PlatonMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, PlatonMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [PlatonMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<PlatonMarkdownTextProps const>(_props);
  ...
  PlatonMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  PlatonMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::PlatonMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::PlatonMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<PlatonMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<PlatonMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<PlatonMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == PlatonMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == PlatonMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == PlatonMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == PlatonMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const PlatonMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (PlatonMarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[PlatonMarkdownTextRun class]]) {
      continue;
    }

    PlatonMarkdownTextRun* textChild = (PlatonMarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::PlatonMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::PlatonMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> PlatonMarkdownTextCls(void)
{
  return PlatonMarkdownText.class;
}
