...
import { HostProcessArchitecture, HostProcessPlatform } from "@platon/shared/hostProcess";
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Platon Code (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Platon Code (Nightly)");
  });
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                PLATON_CODE_DESKTOP_UPDATE_REPOSITORY: "pingdotgg/platon-code",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveGitHubPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                GITHUB_REPOSITORY: "pingdotgg/platon-code",
              },
            }),
          ),
        ),
      ...
      resolveDesktopRuntimeDependencies(
        {
          ...
          "@platon/contracts": "workspace:*",
          "@platon/shared": "workspace:*",
          "@platon/ssh": "workspace:*",
          "@platon/tailscale": "workspace:*",
          effect: "catalog:",
          electron: "41.5.0",
        },
        {
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      // entry advertises MimeType=x-scheme-handler/platon-code; for OAuth deep links.
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "Platon Code", schemes: ["platon-code", "platon-code-dev"] },
      ]);
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      ...
      PLATON_CODE_MACOS_PROVISIONING_PROFILE: "/tmp/platon-code.provisionprofile",
      PLATON_CODE_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });

    assert.deepStrictEqual(configuration, {
      appId: "com.platon.platon-code",
      teamId: "ABC1234567",
      rpDomains: ["example.clerk.accounts.dev"],
      provisioningProfilePath: "/tmp/platon-code.provisionprofile",
    });
  });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      PLATON_CODE_APPLE_TEAM_ID: "ABC1234567",
      PLATON_CODE_MACOS_PROVISIONING_PROFILE: "/tmp/platon-code.provisionprofile",
      PLATON_CODE_CLERK_PASSKEY_RP_DOMAINS:
    ...
    assert.include(entitlements, "<string>ABC1234567.com.platon.platon-code</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    const missingProfileError = captureError({
      PLATON_CODE_APPLE_TEAM_ID: "ABC1234567",
      PLATON_CODE_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev",
    });
    assert.instanceOf(missingProfileError, MissingMacPasskeyProvisioningProfileError);
    assert.equal(
      missingProfileError.message,
      "PLATON_CODE_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
    ...
    const invalidDomainError = captureError({
      PLATON_CODE_APPLE_TEAM_ID: "ABC1234567",
      PLATON_CODE_MACOS_PROVISIONING_PROFILE: "/tmp/platon-code.provisionprofile",
      PLATON_CODE_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          PLATON_CODE_APPLE_TEAM_ID: "ABC1234567",
          PLATON_CODE_MACOS_PROVISIONING_PROFILE: "/tmp/platon-code.provisionprofile",
          PLATON_CODE_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
    ...
    const invalidPublishableKeyError = captureError({
      PLATON_CODE_APPLE_TEAM_ID: "ABC1234567",
      PLATON_CODE_MACOS_PROVISIONING_PROFILE: "/tmp/platon-code.provisionprofile",
      PLATON_CODE_CLERK_PUBLISHABLE_KEY: "pk_test_%",
    });
    ...
    assert.equal(
      ...
      "PLATON_CODE_CLERK_PUBLISHABLE_KEY is invalid.",
    );
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.platon.platon-code");
      assert.equal(mac.entitlements, "/tmp/entitlements.mac.plist");
      assert.equal(mac.provisioningProfile, "/tmp/platon-code.provisionprofile");
  ...
  it("stages the resource monitor as an external executable resource", () => {
    ...
    assert.equal(resourceMonitorExecutableName("mac"), "platon-resource-monitor");
    assert.equal(resourceMonitorExecutableName("win"), "platon-resource-monitor.exe");
  });
      ...
      const resolved = yield* resolveBuildOptions({
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                PLATON_CODE_DESKTOP_SKIP_BUILD: "true",
                PLATON_CODE_DESKTOP_KEEP_STAGE: "true",
                PLATON_CODE_DESKTOP_SIGNED: "true",
                PLATON_CODE_DESKTOP_VERBOSE: "true",
                PLATON_CODE_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
