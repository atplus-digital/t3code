...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "platon-code-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-platon-code", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "platon-code", specifier: pluginPath }],
