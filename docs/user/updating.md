...
asks you to run the exact `npx platon@<version> service update` command on the server machine. This is
...
**Copy update command** gives you `npx platon@<client-version>`, which relaunches the server directly
...
```sh
npx platon@<client-version> service update
```

`service update` installs the version of the CLI that invoked it, so `npx platon@latest service update`
...
3. For a command-line server, relaunch it with `npx platon@<client-version>`, replacing
