// This file mostly exists because we want dev mode to say "Platon Code (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Platon Code (Dev)" : "Platon Code (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.platon.platon-code.dev.${devBundleIdSuffix || "local"}`
  : "com.platon.platon-code";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["platon-code-dev"] : ["platon-code"];
...
// oxlint-disable-next-line platon-code/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["PLATON_CODE_PORT", environment.PLATON_CODE_PORT],
    ["PLATON_CODE_HOME", environment.PLATON_CODE_HOME],
    ["PLATON_CODE_COMMIT_HASH", environment.PLATON_CODE_COMMIT_HASH],
    ["PLATON_CODE_OTLP_TRACES_URL", environment.PLATON_CODE_OTLP_TRACES_URL],
    ["PLATON_CODE_OTLP_EXPORT_INTERVAL_MS", environment.PLATON_CODE_OTLP_EXPORT_INTERVAL_MS],
    ["PLATON_CODE_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --platon-code-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Platon Code name.
