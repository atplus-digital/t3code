...
/**
 * Drives the primary environment's Platon Connect link. Platon Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    ...
    toastManager.add({
      type: "error",
      title: "Could not update Platon Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Platon Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Platon Connect before enabling this."));
        return false;
      }
