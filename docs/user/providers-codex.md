...
This guide is for people who want to use more than one Codex account in Platon Code. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Platon Code](./install.md).
...
In Platon Code Settings, name it something obvious:
...
In Platon Code Settings, add another Codex provider:
...
Platon Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Platon Code treats it
