...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("platon-code-relay-traces", "prod")).toBe(
      "platon-code-relay-traces-prod",
    );
    expect(relayResourceNameForStage("platon-code-relay-traces", "dev_julius")).toBe(
      "platon-code-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "platoncoderelay.com", hash)).toBe(
      "prod-abcdef0123456789.platoncoderelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "platoncoderelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
