...
the software keyboard, and supplies deterministic Platon Connect discovery rows to the real
...
1. Creates three temporary T3 base directories and starts a local server for each on an available
   port.
2. Creates Platon Code, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic Platon Code review diff.
...
ARM64 by default for local Apple Silicon development; CI sets `PLATON_SHOWCASE_ANDROID_ABI=x86_64` so the
...
`pnpm --filter @platon/mobile showcase` starts Expo on its normal port, so it is not compatible with
...
The Pending rows use the production offline outbox and point at the real Platon Code and React fixture
