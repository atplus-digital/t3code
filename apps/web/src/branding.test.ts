...
describe("branding", () => {
  it("uses injected desktop branding when available", async () => {
    Object.defineProperty(globalThis, "window", {
      ...
      value: {
        desktopBridge: {
          getAppBranding: () => ({
            baseName: "Platon Code",
            stageLabel: "Nightly",
            displayName: "Platon Code (Nightly)",
          }),
        },
      },
    });
    ...
    expect(branding.APP_BASE_NAME).toBe("Platon Code");
    expect(branding.APP_STAGE_LABEL).toBe("Nightly");
    expect(branding.APP_DISPLAY_NAME).toBe("Platon Code (Nightly)");
  });
  ...
  it("does not label the latest hosted app channel", async () => {
    ...
    expect(branding.APP_DISPLAY_NAME).toBe("Platon Code");
  });
...
describe("branding logic", () => {
  ...
  it("updates the display name for nightly primary server versions", () => {
    expect(
      resolveServerBackedAppDisplayName({
        baseName: "Platon Code",
        fallbackDisplayName: "Platon Code (Alpha)",
        fallbackStageLabel: "Alpha",
        primaryServerVersion: "0.0.28-nightly.20260616.12",
      }),
    ).toBe("Platon Code (Nightly)");
  });
