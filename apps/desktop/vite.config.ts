...
const shouldLaunchElectronAfterPack = process.env.PLATON_CODE_DESKTOP_DEV === "1";
  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["platon#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-preview-annotation-css.mjs && cross-env PLATON_CODE_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["platon#build"],
        cache: false,
      },
      "dev:bundle": {
  ...
  pack: [
    {
      ...
      clean: true,
      deps: {
        alwaysBundle: (id) => id.startsWith("@platon/"),
      },
