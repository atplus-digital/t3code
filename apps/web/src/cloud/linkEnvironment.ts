...
import { EnvironmentRegistry } from "@platon/client-runtime/connection";
import { request, runStream } from "@platon/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@platon/client-runtime/rpc";
import { ManagedRelay } from "@platon/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Platon Code cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Platon Code cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "PLATON_CODE_RELAY_URL is not configured.",
      });
    }
...
// independently of Platon Connect while clients reach the environment out of band.
