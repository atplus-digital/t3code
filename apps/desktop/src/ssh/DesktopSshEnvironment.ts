...
import * as SshAuth from "@platon/ssh/auth";
import { discoverSshHosts } from "@platon/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@platon/ssh/errors";
import * as SshTunnel from "@platon/ssh/tunnel";
...
>()("@platon/desktop/ssh/DesktopSshEnvironment") {}

export interface DesktopSshEnvironmentLayerOptions {
  readonly resolveCliPackageSpec?: () => string;
  readonly resolveCliRunner?: Effect.Effect<SshTunnel.RemotePlatonRunnerOptions>;
}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Platon Code window is not available for SSH authentication.";
      break;
