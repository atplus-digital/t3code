...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    launcherPath: "/home/theo/.platon/runtime/service-launcher.mjs",
    baseDir: "/home/theo/.platon",
    logPath: "/home/theo/.platon/userdata/logs/boot-service.log",
    unitPath: "/home/theo/.config/systemd/user/platon-code.service",
  });

  expect(unit).toContain("ExecStart=/usr/bin/node /home/theo/.platon/runtime/service-launcher.mjs");
...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  const home = yield* fs.makeTempDirectoryScoped({ prefix: "platon-boot-service-test-" });
  const baseDir = path.join(home, ".platon");
  ...
  it.effect("restarts an installed service when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("systemctl "))).toEqual([
        "systemctl --user stop platon-code.service",
        "systemctl --user daemon-reload",
        "systemctl --user restart platon-code.service",
      ]);
    }),
  );
