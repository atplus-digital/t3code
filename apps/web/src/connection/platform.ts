...
} from "@platon/client-runtime/platform";
import {
...
import { bootstrapRemoteBearerSession } from "@platon/client-runtime/authorization";
import { fetchRemoteEnvironmentDescriptor } from "@platon/client-runtime/environment";
import { managedRelayAccountChanges, managedRelaySessionAtom } from "@platon/client-runtime/relay";
import { EnvironmentRpcRequestObserver } from "@platon/client-runtime/rpc";
import {
...
function clientMetadata() {
  ...
  return {
    label: desktop ? "Platon Code Desktop" : "Platon Code Web",
    ...
    const cloudSession = CloudSession.of({
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Platon Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Platon Connect session is unavailable.",
          });
        }
        return token;
      }),
