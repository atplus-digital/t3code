...
import { resolveGitWorktreePath, resolveWorktreePlatonHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "platon-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreePlatonHome", () => {
  it.effect("answers with .platon before the dev runner creates it", () =>
    Effect.gen(function* () {
      ...
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".platon"));
