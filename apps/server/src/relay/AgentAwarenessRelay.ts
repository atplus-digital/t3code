...
import { projectThreadAwareness } from "@platon/shared/agentAwareness";
import { makeDrainableWorker } from "@platon/shared/DrainableWorker";
...
>()("platon/relay/AgentAwarenessRelay") {}
...
const makePublishProof = Effect.fn("makePublishProof")(function* (input: {
  ...
  const payload = {
    iss: `platon-env:${input.environmentId}`,
      ...
      switch (startupState) {
        case "waiting-for-link":
          yield* Effect.logInfo(
            "agent activity publishing standby; waiting for Platon Connect link reconciliation",
          );
          break;
        case "disabled":
          yield* Effect.logInfo(
            "agent activity publishing disabled by Platon Connect configuration",
          );
          break;
        case "enabled":
