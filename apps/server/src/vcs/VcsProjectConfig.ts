...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@platon/contracts";
import { fromLenientJson } from "@platon/shared/schemaJson";
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".platon-code", "vcs.json");
