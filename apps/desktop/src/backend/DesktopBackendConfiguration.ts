...
>()("@platon/desktop/backend/DesktopBackendConfiguration") {}
...
const DESKTOP_BACKEND_ENV_NAMES = [
  "PLATON_CODE_PORT",
  "PLATON_CODE_MODE",
  "PLATON_CODE_NO_BROWSER",
  "PLATON_CODE_HOST",
  "PLATON_CODE_DESKTOP_WS_URL",
  "PLATON_CODE_DESKTOP_LAN_ACCESS",
  "PLATON_CODE_DESKTOP_LAN_HOST",
  "PLATON_CODE_DESKTOP_HTTPS_ENDPOINTS",
  "PLATON_CODE_TAILSCALE_SERVE",
  "PLATON_CODE_TAILSCALE_SERVE_PORT",
] as const;
...
function resourceMonitorBinaryName(platform: NodeJS.Platform): string {
  return platform === "win32" ? "platon-resource-monitor.exe" : "platon-resource-monitor";
}
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    const bootstrap = {
      ...
      platonHome: environment.baseDir,
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's PLATON_CODE_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  const bootstrap = {
    ...
    noBrowser: true,
    port: input.port,
    // Omit platonHome so the Linux backend uses its own home dir instead of
  ...
  // Build an explicit copy of process.env minus PLATON_CODE_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.platon via
  // /mnt/c, which means both backends read/write the same database and
  // their env-ids collide).
  const parentEnvWithoutPlatonHome: Record<string, string | undefined> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "PLATON_CODE_HOME") continue;
    parentEnvWithoutPlatonHome[key] = value;
  }
  const wslEnv = mergeWslEnv(parentEnvWithoutPlatonHome.WSLENV, forwardedEnvNames);
  ...
  const baseConfig = {
    ...
    cwd: environment.backendCwd,
    env: {
      ...parentEnvWithoutPlatonHome,
      ...backendChildEnvPatch(),
      ...forwardedEnv,
      ...(wslEnv !== undefined ? { WSLENV: wslEnv } : {}),
    },
    // env is already a complete process.env minus PLATON_CODE_HOME; pass it
