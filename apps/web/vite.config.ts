...
import { DEV_PROXIED_PATH_PREFIXES } from "@platon/shared/devProxy";
...
const isSingleOriginDev = process.env.PLATON_CODE_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_PLATON_CODE_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.PLATON_CODE_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: PLATON_CODE_BUNDLED_DEV=1 pnpm dev:web
const bundledDevEnv = process.env.PLATON_CODE_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // PLATON_CODE_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.PLATON_CODE_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.PLATON_CODE_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_PLATON_CODE_RELAY_URL": JSON.stringify(configuredRelayUrl),
