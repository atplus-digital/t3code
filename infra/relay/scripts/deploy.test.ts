...
describe("reconcileRootEnvRelayUrl", () => {
  ...
  it("preserves unrelated root env entries while replacing a previous relay URL", () => {
    expect(
      reconcileRootEnvRelayUrl(
        "PLATON_CODE_CLERK_PUBLISHABLE_KEY=pk_test_example\nPLATON_CODE_RELAY_URL=https://old.example.test\n",
...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "platon-code-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "platon-code-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "PLATON_CODE_MOBILE_OTLP_TRACES_DATASET=platon-code-mobile-traces-dev",
        "PLATON_CODE_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_DATASET=platon-code-relay-client-traces-dev",
        "PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
    ...
    expect(
      reconcileRootEnvPublicConfig(
        [
          "PLATON_CODE_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "PLATON_CODE_RELAY_URL=https://old.example.test",
          "PLATON_CODE_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "PLATON_CODE_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "PLATON_CODE_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
