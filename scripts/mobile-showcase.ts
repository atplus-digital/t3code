...
const ANDROID_PACKAGE = "com.platon.platon-code";
const APP_SCHEME = "platon-code";
const IOS_READY_FILENAME = "PlatonShowcaseReadyScene";
const SERVER_HOST = "0.0.0.0";
const IOS_SIMULATOR_ARCH = NodeProcess.arch === "arm64" ? "arm64" : "x86_64";
const IOS_APP_PATH = NodePath.join(
  MOBILE_ROOT,
  ".showcase/ios-derived-data/Build/Products/Debug-iphonesimulator/PlatonCode.app",
);
...
const MOBILE_BUILD_ENV = {
  ...
  PLATON_SHOWCASE_CAPTURE_BUILD: "1",
  JAVA_HOME:
...
Usage:
  pnpm --filter @platon/mobile screenshots [options]
...
async function buildIos(): Promise<string> {
  ...
  await runCommand(
    ...
    [
      "-workspace",
      NodePath.join(MOBILE_ROOT, "ios/PlatonCode.xcworkspace"),
      "-scheme",
      "PlatonCode",
      "-configuration",
      "Debug",
...
async function captureIos(
  ...
  const scenePath = NodePath.join(
    ...
    "Library/Caches/PlatonShowcaseScene",
  );
  const readyPath = NodePath.join(
    await iosAppContainer(simulator.udid),
    "Library/Caches",
...
async function writeAndroidShowcaseScene(serial: string, scene: ShowcaseScene): Promise<void> {
  await runAdb(serial, [
    "shell",
    `run-as ${ANDROID_PACKAGE} sh -c 'mkdir -p files && rm -f files/platon-showcase-ready && printf %s ${scene} > files/platon-showcase-scene'`,
  ]);
}
...
async function main(): Promise<void> {
  ...
  const showcaseRootDir = await NodeFSP.mkdtemp(
    NodePath.join(NodeOS.tmpdir(), "platon-mobile-showcase-"),
  );
