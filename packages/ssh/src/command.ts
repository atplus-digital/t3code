...
const PUBLISHABLE_PLATON_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemotePlatonCliPackageSpec(input: {
  ...
  if (!input.isDevelopment && PUBLISHABLE_PLATON_VERSION_PATTERN.test(appVersion)) {
    return `platon-code@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "platon-code@nightly";
  }

  return input.updateChannel === "nightly" ? "platon-code@nightly" : "platon-code@latest";
}
