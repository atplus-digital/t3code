...
Platon Code has one server-side observability model:
...
stdout/stderr at `~/.platon/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.platon/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.platon/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.platon/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.platon/dev/logs/server.trace.ndjson`.
...
```bash
export PLATON_CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export PLATON_CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export PLATON_CODE_OTLP_SERVICE_NAME=platon-local
```

Optional:

```bash
export PLATON_CODE_TRACE_MIN_LEVEL=Info
export PLATON_CODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `PLATON_CODE_OTLP_*`.

macOS app bundle example:

```bash
PLATON_CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
PLATON_CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
PLATON_CODE_OTLP_SERVICE_NAME=platon-desktop \
"/Applications/Platon Code.app/Contents/MacOS/Platon Code"
```
...
```bash
TRACE_FILE="${PLATON_CODE_HOME:-$HOME/.platon}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.platon/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.platon/dev/logs/server.trace.ndjson"
```
...
- service name such as `platon-local`, `platon-dev`, or `platon-desktop`
...
- `platon_rpc_request_duration`
- `platon_orchestration_command_duration`
- `platon_orchestration_command_ack_duration`
- `platon_provider_turn_duration`
- `platon_git_command_duration`

Counters tell you volume and failure rate:

- `platon_rpc_requests_total`
- `platon_orchestration_commands_total`
- `platon_provider_turns_total`
- `platon_git_commands_total`
...
`platon_orchestration_command_ack_duration` measures:
...
1. Check `platon_orchestration_command_ack_duration` by `commandType`.
...
- `PLATON_CODE_OTLP_TRACES_URL` was not set
...
- `PLATON_CODE_TRACE_FILE`: override trace file path
- `PLATON_CODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `PLATON_CODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `PLATON_CODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `PLATON_CODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `PLATON_CODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `PLATON_CODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `PLATON_CODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `PLATON_CODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `PLATON_CODE_OTLP_SERVICE_NAME`: service name, default `platon-server`
