...
} from "@platon/shared/httpReadiness";
import * as NetService from "@platon/shared/Net";
import { extractJsonObject, fromLenientJson } from "@platon/shared/schemaJson";
import { satisfiesSemverRange } from "@platon/shared/semver";
...
export interface RemotePlatonRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemotePlatonRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemotePlatonRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  PLATON_NODE_ENGINE_RANGE=@@PLATON_NODE_ENGINE_RANGE@@
  if [ -z "$PLATON_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$PLATON_NODE_ENGINE_RANGE" <<'NODE'
@@PLATON_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for PLATON_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$PLATON_NODE_BIN/node" ]; then
        PATH="$PLATON_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@PLATON_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
PLATON_NODE_SCRIPT_PATH=@@PLATON_NODE_SCRIPT_PATH@@
if [ -n "$PLATON_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$PLATON_NODE_SCRIPT_PATH" "$@"
fi
if command -v t3 >/dev/null 2>&1; then
  exec t3 "$@"
fi
if command -v npx >/dev/null 2>&1; then
  exec npx --yes @@PLATON_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  exec npm exec --yes @@PLATON_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the platon CLI and could not install @@PLATON_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@PLATON_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.platon/ssh-launch/$STATE_KEY"
...
pick_port() {
  node - "$PORT_FILE" "@@PLATON_DEFAULT_REMOTE_PORT@@" "@@PLATON_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@PLATON_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@PLATON_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@PLATON_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@PLATON_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@PLATON_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@PLATON_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env PLATON_CODE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
...
export function buildRemotePlatonRunnerScript(input?: RemotePlatonRunnerOptions): string {
  ...
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      PLATON_PACKAGE_SPEC: packageSpec,
      PLATON_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      PLATON_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemotePlatonRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      PLATON_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      PLATON_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemotePlatonRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    PLATON_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    PLATON_RUNNER_SCRIPT: stripTrailingNewlines(buildRemotePlatonRunnerScript(input)),
    PLATON_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    PLATON_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    PLATON_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    PLATON_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    PLATON_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    PLATON_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    PLATON_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemotePlatonRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    PLATON_STATE_KEY: remoteStateKey(target),
    PLATON_RUNNER_SCRIPT: stripTrailingNewlines(buildRemotePlatonRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemotePlatonRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemotePlatonRunnerOptions;
...
>()("@platon/ssh/tunnel/SshEnvironmentManager") {
