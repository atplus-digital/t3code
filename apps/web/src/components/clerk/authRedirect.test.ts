...
describe("resolveClerkSignInProps", () => {
  ...
  it("omits the redirect override on packaged desktop", () => {
    expect(resolveClerkSignInProps("platon-code://app/#/settings/general", true)).toEqual({});
  });

  it("omits the redirect override on development desktop", () => {
    expect(resolveClerkSignInProps("platon-code-dev://app/#/settings/general", true)).toEqual({});
  });
});
