# Keeping Platon Code in Sync

The Platon Code web or desktop app and the server it connects to work best when they use the same
version. If they do not match, Platon Code shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                                      |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Update server**          | Available for the Platon Code Linux background service. Select the button and leave Platon Code open while it prepares, tests, restarts, and reconnects.                        |
| **Update the desktop app** | Open the Platon Code desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current Platon Code server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. Platon Code does not update connected
servers silently in the background.

If the requested version includes a database update, remote installation stops before restart and
asks you to run the exact `npx platon-code@<version> service update` command on the server machine. This is
...
**Copy update command** gives you `npx platon-code@<client-version>`, which relaunches the server directly
at the matching version. Add whatever startup options you normally use.

If the server instead runs as the Platon Code background service, update the service on the host and
pin the same version:

```sh
npx platon-code@<client-version> service update
```

`service update` installs the version of the CLI that invoked it, so `npx platon-code@latest service update`
...
See [Running Platon Code in the Background](./background-service.md) for install, status, and removal
commands.
...
3. For a command-line server, relaunch it with `npx platon-code@<client-version>`, replacing
