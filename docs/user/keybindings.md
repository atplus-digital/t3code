...
The same configuration lives in `~/.platon/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. Platon Code writes the built-in defaults into that file on
