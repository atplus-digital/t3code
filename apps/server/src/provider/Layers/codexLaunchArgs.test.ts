...
describe("resolveCodexLaunchArgs", () => {
  it("uses PLATON_CODE_CODEX_LAUNCH_ARGS before configured settings", () => {
    NodeAssert.equal(
      resolveCodexLaunchArgs(" --strict-config ", {
        PLATON_CODE_CODEX_LAUNCH_ARGS: "--enable foo",
      }),
      "--enable foo",
    );
  });

  it("uses configured settings when PLATON_CODE_CODEX_LAUNCH_ARGS is empty", () => {
    NodeAssert.equal(
      resolveCodexLaunchArgs(" --strict-config ", { PLATON_CODE_CODEX_LAUNCH_ARGS: "   " }),
      "--strict-config",
    );
  });

  it("ignores whitespace-only environment values", () => {
    NodeAssert.equal(resolveCodexLaunchArgs("", { PLATON_CODE_CODEX_LAUNCH_ARGS: "   " }), "");
  });
});
