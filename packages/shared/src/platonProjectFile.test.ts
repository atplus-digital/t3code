...
import {
  buildPlatonProjectFileJsonSchema,
  PlatonProjectFileFromJson,
} from "./platonProjectFile.ts";

const decodeJson = Schema.decodeUnknownSync(PlatonProjectFileFromJson);

describe("buildPlatonProjectFileJsonSchema", () => {
  it("emits a draft 2020-12 schema with the published $id", () => {
    const schema = buildPlatonProjectFileJsonSchema();

    expect(schema.$schema).toBe("https://json-schema.org/draft/2020-12/schema");
    expect(schema.$id).toBe("https://t3.codes/schema/platon.json");
    expect(schema.type).toBe("object");
    expect(schema.additionalProperties).toBe(false);
  });

  it("documents every supported field", () => {
    const schema = buildPlatonProjectFileJsonSchema() as {
      properties: Record<
        string,
        {
          description?: string;
...
describe("PlatonProjectFileFromJson", () => {
