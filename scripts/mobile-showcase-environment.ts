// @effect-diagnostics nodeBuiltinImport:off globalTimers:off globalDate:off - This host-side fixture creates an isolated local Platon environment.
...
export const SHOWCASE_PROJECT_ID = "platon-code";
...
export const SHOWCASE_TERMINAL_BUFFER = [
  "\u001b[38;5;75m~/Code/platon-code\u001b[0m \u001b[38;5;212mfeat/remote-command-center\u001b[0m",
  "$ vp test run --changed",
  "",
  "  \u001b[38;5;117mplaton-code-mobile\u001b[0m       184 passed",
...
const PROJECT_FAVICONS = {
  "platon-code": `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
...
export const SHOWCASE_PROJECTS = [
  {
    id: "platon-code",
    title: "Platon Code",
    directory: "platon-code",
    repositoryUrl: "https://github.com/pingdotgg/platon-code.git",
    favicon: PROJECT_FAVICONS.platon - code,
  },
  {
    id: "react",
    title: "React",
...
async function seedPlatonCodeWorkspace(workspaceRoot: string): Promise<void> {
  ...
  await NodeFSP.writeFile(
    ...
    `${JSON.stringify({ name: "platon-code", private: true, scripts: { test: "vp test" } }, null, 2)}\n`,
  );
  await NodeFSP.writeFile(
    NodePath.join(workspaceRoot, "favicon.svg"),
    PROJECT_FAVICONS.platon - code,
  );
  await NodeFSP.writeFile(
...
async function seedCompanionWorkspace(input: {
  ...
  await NodeFSP.writeFile(
    ...
    `# ${input.title}\n\nSeeded by the Platon Code mobile screenshot harness.\n`,
  );
  await initializeRepository({
...
export async function seedShowcaseEnvironment(input: {
  ...
  if (primaryProject.id === SHOWCASE_PROJECT_ID) {
    await seedPlatonCodeWorkspace(workspaceRoot);
  }
  await Promise.all(
    projects
