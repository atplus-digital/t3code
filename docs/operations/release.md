...
- Reads the shared production Platon Connect relay URL and Clerk client configuration before packaging clients.
...
- Publishes the CLI package (`apps/server`, npm package `platon`) with OIDC trusted publishing from the same workflow file:
...
## Platon Connect relay deployment
...
```sh
vp run --filter platon-code-relay deploy -- --stage "$USER" --env-file .env.local
```
...
- `PLATON_CODE_WEB_ROUTER_URL`: defaults to `https://app.t3.codes`.
- `PLATON_CODE_WEB_LATEST_DOMAIN`: defaults to `latest.app.t3.codes`.
- `PLATON_CODE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.t3.codes`.
...
visiting `/__platon-code/channel?channel=latest` or
`/__platon-code/channel?channel=nightly`; the router stores the
`platon-code_web_channel` cookie and rewrites future requests on `app.t3.codes` to
...
through `/__platon-code/channel` on the router domain so the user's channel cookie is
...
- Publishes the CLI package (`apps/server`, npm package `platon`) to the `nightly` npm dist-tag using the same nightly version.
...
For a release smoke test, confirm `npm view platon@<version> version` returns the expected version, then
...
- Repository slug source:
  - `PLATON_CODE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
...
1. Confirm npm org/user owns package `platon` (or rename package first if needed).
...
`v0.0.0-test.1`, classifies the run as the stable channel. It publishes `platon` with npm dist-tag
...
2. Create an explicit App ID for `com.platon.platon-code` and enable Associated Domains.
...
10. Complete the Clerk Native API and AASA setup in [Platon Connect Clerk Setup](../internals/platon-connect.md#desktop-passkeys).
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.com.platon.platon-code` and includes
    Associated Domains.
