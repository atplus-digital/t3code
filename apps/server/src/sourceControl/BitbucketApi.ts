...
import { sanitizeBranchFragment } from "@platon/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@platon/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("PLATON_CODE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("PLATON_CODE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("PLATON_CODE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("PLATON_CODE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("platon/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `platon-code/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set PLATON_CODE_BITBUCKET_EMAIL and PLATON_CODE_BITBUCKET_API_TOKEN, or PLATON_CODE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
