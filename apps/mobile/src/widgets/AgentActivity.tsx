...
export function AgentActivity(
  ...
  const deepLink =
    deepLinkRow && deepLinkRow.deepLink.startsWith("/") && !deepLinkRow.deepLink.startsWith("//")
      ? `platon-code://${deepLinkRow.deepLink.slice(1)}`
      : null;
  ...
  // The branded Platon temple mark. `assetName` resolves the template image set
  // bundled in the widget extension's asset catalog. Image views only honor `resizable`
  ...
  const renderLogo = (height: number, color: string) => (
    <HStack modifiers={[frame({ width: height * 1.5, height }), foregroundStyle(color)]}>
      <Image assetName="PlatonMark" modifiers={[resizable()]} />
    </HStack>
  );
