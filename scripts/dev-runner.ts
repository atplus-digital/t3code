...
import * as NetService from "@platon/shared/Net";
import { resolveGitWorktreePath, resolveWorktreePlatonHome } from "@platon/shared/devHome";
import { HostProcessEnvironment, HostProcessWorkingDirectory } from "@platon/shared/hostProcess";
...
export const DEFAULT_PLATON_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".platon"),
);
...
/**
 * `role` matters because only the backend honours `--host`/`PLATON_CODE_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("PLATON_CODE_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("PLATON_CODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("PLATON_CODE_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "PLATON_CODE_PORT_OFFSET",
    ...
    return Effect.succeed({
      ...
      source: `PLATON_CODE_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric PLATON_CODE_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed PLATON_CODE_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_PLATON_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly platonHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .platon > ambient PLATON_CODE_HOME) is resolved
    // by the caller; an unset platonHome here genuinely means "use the default".
    const configuredBaseDir = platonHome?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.PLATON_CODE_HOME = resolvedBaseDir;
    } else {
      delete output.PLATON_CODE_HOME;
    }

    if (!isDesktopMode) {
      output.PLATON_CODE_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.PLATON_CODE_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.PLATON_CODE_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.PLATON_CODE_PORT = String(serverPort);
      ...
      delete output.PLATON_CODE_SINGLE_ORIGIN_DEV;
      delete output.PLATON_CODE_MODE;
      delete output.PLATON_CODE_NO_BROWSER;
      delete output.PLATON_CODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.PLATON_CODE_HOST = host;
    }

    if (!isDesktopMode) {
      output.PLATON_CODE_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.PLATON_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.PLATON_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.PLATON_CODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.PLATON_CODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.PLATON_CODE_MODE = "web";
      delete output.PLATON_CODE_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`PLATON_CODE_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["PLATON_CODE_PORT_OFFSET", "PLATON_CODE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.platon` — see @platon/shared/devHome for why this must
    // outrank an ambient PLATON_CODE_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreePlatonHome(yield* HostProcessWorkingDirectory);
    ...
    const resolvedPlatonHome =
      (input.platonHome?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.PLATON_CODE_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      platonHome: resolvedPlatonHome,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.PLATON_CODE_HOME ?? (yield* DEFAULT_PLATON_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.PLATON_CODE_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.PLATON_CODE_DEV_ALLOWED_ORIGINS = [
            env.PLATON_CODE_DEV_ALLOWED_ORIGINS,
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to PLATON_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("PLATON_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription(
      "WebSocket event logging toggle (equivalent to PLATON_CODE_LOG_WS_EVENTS).",
    ),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("PLATON_CODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to PLATON_CODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("PLATON_CODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to PLATON_CODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("PLATON_CODE_PORT")),
  ),
