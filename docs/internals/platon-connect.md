...
Platon Connect uses one Clerk application for web, desktop, and mobile authentication. The relay verifies
two kinds of bearer credential: template JWTs generated from the `platon-relay` template with the shared
`platon-code-relay` audience, and Clerk OAuth tokens issued to the CLI. `verifyRelayClientBearerToken` in
...
[platon-code-connect-auth-flow.html](./platon-code-connect-auth-flow.html).

## Application Keys

Platon Connect is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:

```dotenv
PLATON_CODE_CLERK_PUBLISHABLE_KEY=<publishable key>
PLATON_CODE_CLERK_JWT_TEMPLATE=<JWT template name>
PLATON_CODE_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
PLATON_CODE_RELAY_URL=https://relay.example.com
```
...
should set `PLATON_CODE_CLERK_PUBLISHABLE_KEY`, `PLATON_CODE_CLERK_JWT_TEMPLATE`,
`PLATON_CODE_CLERK_CLI_OAUTH_CLIENT_ID`, and `PLATON_CODE_RELAY_URL` before building. EAS preview and
production builds only need the Clerk publishable key, JWT template name, and relay URL in their EAS
environment.

When any client-facing public value is absent, cloud UI is omitted. The `platon connect` command group is
...
deployment defaults.
`vp run --filter platon-code-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
The `platon connect` commands authorize a headless environment with a separate Clerk OAuth application.
...
3. Add **both** allowed redirect URIs:
   ...
   - `https://app.t3.codes/connect/callback` for the hosted out-of-band flow. This is
     ...
     custom `PLATON_CODE_HOSTED_APP_URL` means `$PLATON_CODE_HOSTED_APP_URL/connect/callback` instead.
     Omitting it breaks headless and SSH authorization.
4. Enable the `openid`, `profile`, and `email` scopes.
5. Set `PLATON_CODE_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
```sh
platon connect            # default: onboarding
platon connect login
platon connect link       # --publish-only
platon connect status     # --json
platon connect publish    # --disable
platon connect unlink
platon connect logout
```

`platon serve` is a separate top-level command, not a connect subcommand.

`platon connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `platon connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running T3 server. The next `platon serve` or `platon start` reconciles the relay link and launches the
managed tunnel. `platon connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `platon connect link` can re-enable exposure without another browser flow. `platon connect
...
logout leaves it running; manage it with `platon service status`, `install`, `update`, and `uninstall`.
...
| ------- | -------------------------------- |
| Name    | `platon-relay`                   |
| Claims  | `{ "aud": "platon-code-relay" }` |

Set `PLATON_CODE_CLERK_JWT_TEMPLATE=platon-relay` in the repository-root `.env`, and set
`CLERK_JWT_AUDIENCE=platon-code-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `PLATON_CODE_RELAY_URL` still
...
Local desktop development uses `platon-code-dev://app`, while packaged builds use `platon-code://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `platon-code-dev://app`; the production Clerk instance should only need
`platon-code://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["platon-code://app"]}'
```
...
The production macOS bundle ID is `com.platon.platon-code`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.platon.platon-code` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.platon.platon-code`.
...
```dotenv
PLATON_CODE_APPLE_TEAM_ID=ABC1234567
PLATON_CODE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/platon-code.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
PLATON_CODE_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `PLATON_CODE_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`PLATON_CODE_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `PLATON_CODE_PORT` set. Rebuild the signed app
...
PLATON_CODE_PORT=13773 \
  "/Applications/Platon Code (Alpha).app/Contents/MacOS/Platon Code (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/Platon Code (Alpha).app"
codesign -d --entitlements :- "/Applications/Platon Code (Alpha).app"
```
...
Signed-in users manage Platon Connect under **Connections**. The settings sidebar also has dedicated
controls, rendered by `SettingsSidebarNav.tsx`: `PlatonConnectSidebarSignIn` in the footer shows a
**Sign in to Platon Connect** button while signed out, and `PlatonConnectSidebarAvatar` shows a Clerk
