...
}).annotate({ description: "Links an authenticated cloud user to a Platon environment." });
...
>()("@platon/contracts/relay/RelayClientPrincipal") {}
...
>()("@platon/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Platon Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["platon-mobile", "platon-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "platon-mobile" as const;
export const RelayWebClientId = "platon-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Platon Code Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Platon environments, connecting authorized clients, and publishing agent activity.",
  );
