...
const OVERLAY_ATTRIBUTE = "data-platon-code-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--platon-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--platon-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--platon-radius": theme.radius,
    "--platon-background": theme.background,
    "--platon-foreground": theme.foreground,
    "--platon-popover": theme.popover,
    "--platon-popover-foreground": theme.popoverForeground,
    "--platon-primary": theme.primary,
    "--platon-primary-foreground": theme.primaryForeground,
    "--platon-muted": theme.muted,
    "--platon-muted-foreground": theme.mutedForeground,
    "--platon-accent": theme.accent,
    "--platon-accent-foreground": theme.accentForeground,
    "--platon-border": theme.border,
    "--platon-input": theme.input,
    "--platon-ring": theme.ring,
    "--platon-font-sans": theme.fontSans,
    "--platon-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  cursorStyle.textContent = `html[data-platon-code-annotation-tool] body, html[data-platon-code-annotation-tool] body * { cursor: crosshair !important; } [${OVERLAY_ATTRIBUTE}], [${OVERLAY_ATTRIBUTE}] * { cursor: default !important; } [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-inner-spin-button, [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-outer-spin-button { appearance:none; margin:0; }`;
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-platon-code-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--platon-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-platon-code-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
