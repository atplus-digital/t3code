...
public final class PlatonNativeControlsModule: Module {
  public func definition() -> ModuleDefinition {
    ...
    Function("getShowcaseScene") { () -> String? in
      let scenePath = NSHomeDirectory() + "/Library/Caches/PlatonShowcaseScene"
    ...
    AsyncFunction("applyShowcaseOrientation") { (orientation: String) in
      ...
      for case let windowScene as UIWindowScene in UIApplication.shared.connectedScenes {
        windowScene.requestGeometryUpdate(.iOS(interfaceOrientations: mask)) { error in
          NSLog("PlatonNativeControls applyShowcaseOrientation(\(orientation)) failed: \(error)")
        }
    ...
    Function("markShowcaseReady") { (scene: String) in
      let readyPath = NSHomeDirectory() + "/Library/Caches/PlatonShowcaseReadyScene"
      try? scene.write(toFile: readyPath, atomically: true, encoding: .utf8)
    }

    View(PlatonHeaderButtonView.self) {
      Prop("label") { (view: PlatonHeaderButtonView, label: String) in
        view.setLabel(label)
      }
      Prop("systemImage") { (view: PlatonHeaderButtonView, systemImage: String) in
