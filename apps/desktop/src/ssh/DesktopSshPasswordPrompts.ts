import type { DesktopSshPasswordPromptRequest } from "@platon/contracts";
import { DesktopSshPasswordPromptResolutionInputSchema } from "@platon/contracts";
import type { SshPasswordRequest } from "@platon/ssh/auth";
...
export class DesktopSshPromptWindowUnavailableError extends Schema.TaggedErrorClass<DesktopSshPromptWindowUnavailableError>()(
  ...
  override get message(): string {
    ...
    return `Platon Code window is unavailable during ${this.stage} for SSH authentication to ${this.destination} (request: ${request}).`;
  }
}
...
>()("@platon/desktop/ssh/DesktopSshPasswordPrompts") {}
