...
import { stableStringify } from "@platon/shared/relaySigning";
...
>()("platon-code-relay/environments/EnvironmentConnector") {}
...
function verifyEnvironmentResponse(input: {
  ...
  return verifyWithEnvironmentKeys({
    ...
    issuer: `platon-env:${input.environmentId}`,
...
const make = Effect.gen(function* () {
  ...
  return EnvironmentConnector.of({
    status: Effect.fn("relay.environment_connector.status")(function* (input) {
      ...
      const payload = {
        iss: relayIssuer,
        aud: `platon-env:${link.environmentId}`,
        sub: input.userId,
    ...
    connect: Effect.fn("relay.environment_connector.connect")(function* (input) {
      ...
      const decoded = yield* environmentClient.connect
        .platonMintCredential({ payload: { proof } })
        .pipe(
          withoutRedirects,
          Effect.mapError(
            (cause) =>
