...
export interface PlatonCodePublicConfig {
...
export function loadRepoEnv({
  ...
  return {
    ...
    ...(config.clerkPublishableKey
      ? {
          PLATON_CODE_CLERK_PUBLISHABLE_KEY: config.clerkPublishableKey,
    ...
    ...(config.clerkJwtTemplate
      ? {
          PLATON_CODE_CLERK_JWT_TEMPLATE: config.clerkJwtTemplate,
    ...
    ...(config.clerkCliOAuthClientId
      ? {
          PLATON_CODE_CLERK_CLI_OAUTH_CLIENT_ID: config.clerkCliOAuthClientId,
          VITE_CLERK_CLI_OAUTH_CLIENT_ID: config.clerkCliOAuthClientId,
        }
      : {}),
    ...(config.relayUrl
      ? {
          PLATON_CODE_RELAY_URL: config.relayUrl,
          VITE_PLATON_CODE_RELAY_URL: config.relayUrl,
        }
      : {}),
    ...(config.mobileOtlpTracesUrl
      ? {
          PLATON_CODE_MOBILE_OTLP_TRACES_URL: config.mobileOtlpTracesUrl,
          EXPO_PUBLIC_OTLP_TRACES_URL: config.mobileOtlpTracesUrl,
        }
      : {}),
    ...(config.mobileOtlpTracesDataset
      ? {
          PLATON_CODE_MOBILE_OTLP_TRACES_DATASET: config.mobileOtlpTracesDataset,
          EXPO_PUBLIC_OTLP_TRACES_DATASET: config.mobileOtlpTracesDataset,
        }
      : {}),
    ...(config.mobileOtlpTracesToken
      ? {
          PLATON_CODE_MOBILE_OTLP_TRACES_TOKEN: config.mobileOtlpTracesToken,
          EXPO_PUBLIC_OTLP_TRACES_TOKEN: config.mobileOtlpTracesToken,
        }
      : {}),
    ...(config.relayClientOtlpTracesUrl
      ? {
          PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_URL: config.relayClientOtlpTracesUrl,
          VITE_RELAY_OTLP_TRACES_URL: config.relayClientOtlpTracesUrl,
        }
      : {}),
    ...(config.relayClientOtlpTracesDataset
      ? {
          PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.relayClientOtlpTracesDataset,
          VITE_RELAY_OTLP_TRACES_DATASET: config.relayClientOtlpTracesDataset,
        }
      : {}),
    ...(config.relayClientOtlpTracesToken
      ? {
          PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.relayClientOtlpTracesToken,
...
export function resolvePublicConfig(...sources: readonly Environment[]): PlatonCodePublicConfig {
  return {
    ...
    clerkJwtTemplate: firstNonEmpty(
      sources,
      "PLATON_CODE_CLERK_JWT_TEMPLATE",
