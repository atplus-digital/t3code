...
This guide is for people who want to use more than one Claude setup in Platon Code. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Platon Code](./install.md).
...
In Platon Code Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Platon Code uses Claude Code's normal config directory.

When you set this field, Platon Code points Claude Code at that directory with the
...
`~/.claude_personal_home/.claude`, which is not where Platon Code looks.

Then add another Claude provider in Platon Code:
...
Platon Code only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Platon Code keeps separate config directories isolated
...
Add or edit a Claude provider in Platon Code Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Platon Code stores the value as a server secret and does not
...
Platon Code does not need a special Claude Code Router provider. Treat the router as a Claude
