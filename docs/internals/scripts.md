...
Platon Code uses [Vite+](https://viteplus.dev/guide/). Install the global `vp` command, install
...
- `vp run dev --browser`: Auto-opens a browser. Off by default. The dev runner writes
  `PLATON_CODE_NO_BROWSER` itself from this flag, so setting `PLATON_CODE_NO_BROWSER=0` in your environment has
...
- Pass dev-runner flags directly after the root task name, for example:
  `vp run dev --home-dir /tmp/platon-code-dev`

### Dev state directories

- Dev commands run from a linked **git worktree** default to that worktree's gitignored `.platon`, even
  when `PLATON_CODE_HOME` is set, storing state in `<worktree>/.platon/userdata`. Pass `--home-dir <path>` to
  choose another isolated directory explicitly. Submodules are not worktrees and keep the normal
  precedence.
- From the **main checkout**, dev commands implicitly use `~/.platon/dev`, keeping development state
  separate from `~/.platon/userdata`. An explicit `--home-dir <path>` stores state under
  `<path>/userdata`; the base directory remains available for caches, worktrees, and other shared
  data.

## Build, check, test

- `vp run build`: Fans out over `apps/*`, `packages/*`, `oxlint-plugin-platon-code`, and `scripts`.
...
- `node apps/server/scripts/platon-sqlite-state.ts <query|exec> --base-dir <path> ...`: Inspects or seeds
...
- Desktop production windows load the bundled UI from the `platon-code://app/` root URL (not a
  `127.0.0.1` document URL, and not an explicit `index.html` path).
- Desktop packaging includes `apps/server/dist` (the `platon` backend) and starts it on loopback with an
...
- Signed macOS builds also require `PLATON_CODE_APPLE_TEAM_ID` and
  `PLATON_CODE_MACOS_PROVISIONING_PROFILE`. The passkey RP domain is derived from
  `PLATON_CODE_CLERK_PUBLISHABLE_KEY` unless `PLATON_CODE_CLERK_PASSKEY_RP_DOMAINS` overrides it.
...
- Example: `PLATON_CODE_DEV_INSTANCE=branch-a vp run dev:desktop`

Offset resolution, in order:

1. `PLATON_CODE_PORT_OFFSET`, which must be a non-negative integer. Negative values are rejected.
2. `PLATON_CODE_DEV_INSTANCE`. An all-digit value is used directly as the offset; any other non-empty
