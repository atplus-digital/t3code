...
const HOSTED_WEB_CHANNEL_COOKIE = "platon-code_web_channel";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @platon/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --ignore-scripts --filter '@platon/scripts...' --filter '@platon/web...'",
  routes: [
    {
      src: "/__platon-code/channel",
