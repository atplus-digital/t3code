...
import type { RelayClientEnvironmentRecord } from "@platon/contracts/relay";
...
/**
 * The user's Platon Connect environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through Platon Connect.`,
      });
      return;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the Platon Connect environment.";
    const traceId = findErrorTraceId(cause);
    console.error("[platon-connect] Could not connect environment", { message, traceId, cause });
    toastManager.add({
      type: "error",
      ...
      return (
        <div className={ITEM_ROW_CLASSNAME}>
          <p className="text-sm font-medium text-destructive">
            Could not load Platon Connect environments
          </p>
