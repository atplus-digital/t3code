  ...
  ProcessRunner.ProcessRunner.of({
    run: (input) =>
      Effect.gen(function* () {
        ...
        const entry = path.join(stagingDir, "node_modules", "platon-code", "dist", "bin.mjs");
...
it.layer(NodeServices.layer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("validates a staging tree before atomically publishing it", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "platon-pinned-runtime-test-" });
  ...
  it.effect("replaces an incomplete pinned runtime", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "platon-pinned-runtime-repair-",
      });
      const finalPaths = pinnedRuntimePaths(path, baseDir, "1.2.3");
  ...
  it.effect("removes staging when installation is interrupted", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "platon-pinned-runtime-interrupt-",
      });
