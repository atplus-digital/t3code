...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    PLATON_CODE_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    PLATON_CODE_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    PLATON_CODE_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("PLATON_CODE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("PLATON_CODE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "PlatonCodeRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Platon Code relay through Alchemy."));
