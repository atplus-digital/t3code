import { HostProcessExecutablePath, HostProcessPlatform } from "@platon/shared/hostProcess";
...
const BOOT_SERVICE_NAME = "platon-code";
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    "[Unit]",
    "Description=Platon Code server",
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=PLATON_CODE_HOME=${quoteSystemdValue(plan.baseDir)}`,
...
export class BootServiceInstallError extends Schema.TaggedErrorClass<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the Platon Code background service.";
  }
}
